...
public class Day09Part2 {
		...
		for (int y = 1; y <= maxY; y++) {
			for (int x = 1; x <= maxX; x++) {
				...
				if (leftValue > height && rightValue > height && topValue > height && bottomValue > height) {
					int basin = findBasin(heightMap, y, x);
					basins.add(basin);
				}
			}
		}
	...
	private static int findBasin(final int[][] heightMap, final int y, final int x) {
		if (y >= heightMap.length - 1 || y < 1 || x >= heightMap[y].length - 1 || x < 1 || heightMap[y][x] == 9) {
			return 0;
		}
		...
		int result = 1;
		result += findBasin(heightMap, y, x - 1);
		result += findBasin(heightMap, y, x + 1);
		result += findBasin(heightMap, y - 1, x);
		result += findBasin(heightMap, y + 1, x);
